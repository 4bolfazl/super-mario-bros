...
public class FireBarBlockObject extends BlockObject {
    @JsonIgnore
    public BufferedImage image = ImageLoader.getInstance().getBlockImage(BlockType.SIMPLE);

    public FireBarBlockObject(int x, int y, BlockType blockType, ItemType item) {
        super(x, y, blockType, item);
    }

    public FireBarBlockObject(int x, int y, BlockType blockType) {
        super(x, y, blockType);
    }

    public FireBarBlockObject() {
    }
